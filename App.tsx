...
const SidebarLink = ({ icon: Icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${active ? 'bg-indigo-50 text-indigo-700 font-bold shadow-sm' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-900'}`}>
    ...
    <span className="hidden lg:block whitespace-nowrap uppercase text-[10px] font-black tracking-widest">{label}</span>
  </button>
);
...
const App: React.FC = () => {
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 selection:bg-indigo-500/30 font-sans text-white">
        <div className="relative bg-white/5 backdrop-blur-xl p-10 rounded-[3rem] shadow-2xl border border-white/10 w-full max-w-lg text-center">
           <div className="w-24 h-24 bg-indigo-600 rounded-[2rem] flex items-center justify-center mx-auto mb-8 shadow-2xl">
             <PieChart size={48} className="text-white" />
           </div>
           <h1 className="text-4xl font-extrabold mb-10 tracking-tight uppercase">AdTracker Sênior</h1>
           <div className="grid grid-cols-1 gap-4 text-left">
             {ALLOWED_USERS.map(user => (
               <button 
                key={user} 
                onClick={() => handleLogin(user)}
                className="flex items-center justify-between p-6 rounded-[1.5rem] bg-white/5 border border-white/10 hover:border-indigo-500 hover:bg-indigo-600/10 transition-all group"
               >
                 <span className="font-bold text-xl uppercase">{user}</span>
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 flex font-sans">
      <aside className="w-20 lg:w-72 bg-white border-r border-gray-200 flex flex-col fixed h-full z-10 shadow-sm">
        <div className="p-8 flex items-center gap-4 border-b border-gray-100">
          <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center"><PieChart size={24} className="text-white" /></div>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50/50">
          <div className="hidden lg:flex flex-col gap-3 p-4 bg-white rounded-2xl border border-gray-200 shadow-sm mb-4">
             <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-lg">{currentUser[0]}</div>
          ...
          <SidebarLink icon={Settings} label="Ajustes" active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} />
        </div>
      </aside>
      ...
      <main className="flex-1 ml-20 lg:ml-72 p-6 lg:p-12">
        ...
        {activeTab === 'dashboard' && (
          <Dashboard 
            ads={ads} expenses={expenses} recurringExpenses={recurringExpenses} offers={offers} 
            selectedOfferId={selectedOfferId} setSelectedOfferId={setSelectedOfferId}
